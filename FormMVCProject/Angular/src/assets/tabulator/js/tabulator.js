...
(function (global, factory) {
                    ...
                    //if (!filterFunc) {
                        switch (filterType) {
                            case "partial":
                                ...
                                filterFunc = function filterFunc(data) {
                                    var userValue = String(value);
                                    var userValues = userValue.split(',');
                                    var userArray = userValues.map(function (value) {
                                        return $.trim(value);
                                    });
                                    if (userArray.length > 1) {
                                        var dataValue = String(column.getFieldValue(data)).toLowerCase().replace(/<\/?[^>]+(>|$)/g, "");
                                        //return userArray.indexOf(dataValue) > -1;
                                        var found = -1;
                                        userArray.forEach(function (a) {
                                            if (a !== '' && ((a.indexOf(dataValue) > -1) || (dataValue.indexOf(a) > -1)))
                                                found++;
                                        });
    ...
    if(pathArray.length){
        ...
        //tabulatorFilter = 'tabulator-header-filter-' + lastValue;
        if(isNaN(lastValue)){
        ...
        if(tabulatorFilter !== ''){
            var filterArr = localStorage.getItem(tabulatorFilter);
            if(filterArr === null){
                var filterArr = {};
                filterArr['name'] = '';
                filterArr['filters'] = {};
            ...
            if (editor) {
                ...
                searchTrigger = function searchTrigger(e) {
                    ...
                    typingTimer = setTimeout(function () {
                        if(tabulatorFilter !== ''){                                
                            ...
                            filterArr['filters'][column.definition.field] = editorElement.value;
                            localStorage.setItem(tabulatorFilter, JSON.stringify(filterArr));
                        }
                        //console.log(localStorage.getItem(tabulatorFilter));
                ...
                if (column.definition.headerFilterLiveFilter !== false) {
                    ...
                    if (column.modules.filter.tagType == "input" || column.modules.filter.tagType == "select" || column.modules.filter.tagType == "textarea") {
                        ...
                        editorElement.addEventListener("keyup", searchTrigger);
                        editorElement.addEventListener("blur", searchTrigger);
                    }
                }
