...
function getTabulatorGroupBy(tabulatorID, persistenceID){
...
function setTabulatorGroupBy(tabulatorID, persistenceID, columnName){
    ...
    if($("." + columnName).hasClass("col-hide")){        
        $tabulator.tabulator("setGroupBy", columnName);
        $tabulator.tabulator("clearSort");
        $tabulator.tabulator("setSort", columnName, "asc");
        $.cookie(cookieName, columnName);
    }else{
        $("." + columnName).removeClass("col-hide active");
        $tabulator.tabulator("setGroupBy", false);
        $.cookie(cookieName, "");
    }
    $('#pageActionModal').modal('hide');
}